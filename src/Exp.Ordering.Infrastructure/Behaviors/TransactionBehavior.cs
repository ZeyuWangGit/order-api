        ...
        public async Task<TResponse> Handle(TRequest request, CancellationToken cancellationToken, RequestHandlerDelegate<TResponse> next)
        {
            ...
            try
            {
                ...
                await strategy.ExecuteAsync(async () =>
                {
                    ...
                    {
                        _logger.LogInformation("----- Transaction Started {TransactionId} ({@Command})", transaction.TransactionId, typeName, request);

                        response = await next();

                        _logger.LogInformation("----- Transaction Committed {TransactionId} {CommandName}", transaction.TransactionId, typeName);
            ...
            {
                _logger.LogError(ex, "Exception in transaction {CommandName} ({@Command})", typeName, request);
